...
from dotenv import (
    load_dotenv,
)  # Needed to load the environment variables from the .env file
...
db_file = (
    f"{studentid}.db"  # Use this variable when referencing the SQLite database file.
)
...
def insert_dict_into_table(conn, table_name, data_dict):
    ...
    placeholders = ":" + ", :".join(
        data_dict.keys()
    )  # Named placeholders for each key in the dictionary

    # Check if stop already exists (using stop_id for uniqueness)
    cursor = conn.cursor()
    cursor.execute(
        f"SELECT * FROM {table_name} WHERE stop_id=?", (data_dict["stop_id"],)
    )
...
def get_nearby_stop(stop_id):
    ...
    cursor.execute(
        "SELECT stop_id FROM stops WHERE stop_id > ? ORDER BY stop_id LIMIT 1",
        (stop_id,),
    )
    next_stop = cursor.fetchone()
    next_stop_id = next_stop[0] if next_stop else None
    cursor.execute(
        "UPDATE stops SET next_stop = ? WHERE stop_id = ?", (next_stop_id, stop_id)
    )
    db.commit()

    # Query for the previous stop
    cursor.execute(
        "SELECT stop_id FROM stops WHERE stop_id < ? ORDER BY stop_id DESC LIMIT 1",
        (stop_id,),
    )
    prev_stop = cursor.fetchone()
    prev_stop_id = prev_stop[0] if prev_stop else None
    cursor.execute(
        "UPDATE stops SET prev_stop = ? WHERE stop_id = ?", (prev_stop_id, stop_id)
    )
    db.commit()

    db.close()

    return {"output_next": next_stop_id, "output_prev": prev_stop_id}
    ...
    def put(self):
        ...
        try:
            ...
            if stops_data:
                ...
                print(
                    "Deutsche Bahn API returned no stops for the query."
...
def get_next_departure_from_api(stop_id):
    ...
    for departure in departures["departures"]:
        if (
            "platform" in departure
            and departure["platform"] is not None
            and "direction" in departure
            and departure["direction"] is not None
        ):
            try:
                ...
                print(
                    "Error: Station information unavailable for the first departure with valid platform and direction."
                )
                break
...
class Stop(Resource):
    def get(self, stop_id):
        ...
        include_fields = (
            params.get("include").split(",") if params.get("include") else None
        )
        ...
        if "next_departure" in stop_dict:
            cursor.execute(
                "UPDATE stops SET next_departure = ? WHERE stop_id = ?",
                (stop_dict["next_departure"], stop_id),
            )
            conn.commit()
        ...
        links = {
            ...
            "next": (
                "http://{}:{}/stops/{}".format(
                    ...
                    app.config["PORT"],
                    nearby_stop["output_next"],
                )
                if nearby_stop["output_next"]
                else None
            ),
            "prev": (
                "http://{}:{}/stops/{}".format(
                    app.config["HOST_NAME"],
                    app.config["PORT"],
                    nearby_stop["output_prev"],
                )
                if nearby_stop["output_prev"]
                else None
            ),
        }
    ...
    def patch(self, stop_id):
        ...
        if (
            "name" in update_data
            and not isinstance(update_data["name"], str)
            or not update_data["name"].strip()
        ):
            print("Invalid name - must be a non-empty string")
            return {"error": "Invalid name - must be a non-empty string"}, 400
        if "latitude" in update_data and (
            not isinstance(update_data["latitude"], float)
            or update_data["latitude"] < -90
            or update_data["latitude"] > 90
        ):
            print(
                "Invalid latitude - must be a valid floating-point value between -90 and 90"
            )
            return {
                "error": "Invalid latitude - must be a valid floating-point value between -90 and 90"
            }, 400
        if "longitude" in update_data and (
            not isinstance(update_data["longitude"], float)
            or update_data["longitude"] < -180
            or update_data["longitude"] > 180
        ):
            print(
                "Invalid longitude - must be a valid floating-point value between -180 and 180"
            )
            return {
                "error": "Invalid longitude - must be a valid floating-point value between -180 and 180"
            }, 400
        ...
        if update_fields:
            ...
            cursor.execute(
                "SELECT stop_id, last_updated FROM stops WHERE stop_id = ?", (stop_id,)
            )
...
def get_operator_information(operator_name):
    ...
    if response:
        return (
            response.text.strip()
        )  # Assuming information is in the plain text response
    else:
        print(
            f"Error fetching information for {operator_name}: {response.error_message()}"
        )
        return None
        ...
        processed_profiles.append(
            {
                ...
                "information": (
                    information if information else "Information not available"
                ),
            }
        )

    return {"profiles": processed_profiles}
